...
private:
    ...
    void fungsi_subscribe(const sensor_msgs::msg::Joy &msg)
    {
        ...
        // Format string yang mewakili tiap gerakan
        std::string data = format_command_string(cmd);

        // Kirim data ke STM32 melalui serial
        send_to_stm32(data);
    }

    std::string format_command_string(const stmface::msg::Command &cmd)
    {
        // Format string dengan 1 karakter per gerakan
        char buffer[128];
        snprintf(buffer, sizeof(buffer), 
                 "X%.2f,Y%.2f,D%.2f,W%.2f,A%.0f,B%.0f,X%.0f,Y%.0f",
                 cmd.x_cmd, cmd.y_cmd, cmd.depth, cmd.yaw,
                 cmd.a, cmd.b, cmd.x, cmd.y);
        return std::string(buffer);
    }

    void send_to_stm32(const std::string &data)
    {
        try
        {
